							...
							<form name="s" method = "post">
								<div class="row">
									...
									<div class="col-sm-12">
										...
										<input type="search" id="searchUser" name="search"
								...
								<div class="row" cellspacing="0">
									<div class="col-sm-12" id="tables">
										<div class="row" id="tabletitle">
											...
											<div class="col-sm-5">주소</div>
											<div class="col-sm-2">탈퇴여부</div>
										...
										<div class="row" id="tablebody">
											...
											<div class="col-sm-2"><%=CmmUtil.nvl(rDTO.getUser_id())%></div>
											<div class="col-sm-2"><%=CmmUtil.nvl(rDTO.getNick_name())%></div>
											<div class="col-sm-5"><%=CmmUtil.nvl(rDTO.getAddr1())%></div>
											<div class="col-sm-2">
												<button class="btn-dark btn-simple caret" type="button"
													onclick="dropUser(<%=CmmUtil.nvl(rDTO.getUser_no())%>)">탈퇴</button>
											</div>
											<hr style="width:100%;" />
										</div>
...
<script>
	function enter() {
		...
		if (search == "") {
			document.getElementById("searchNotice").value = null;
			s.action="/manage.do";
			s.submit();
		} else{
...
<script type="text/javascript">
	function dropUser(user_no){
		if (confirm("해당 회원을 탈퇴시키겠습니까?") == true){ //동의
			location.href='/dropUser.do?user_no='+ user_no;	
		}
